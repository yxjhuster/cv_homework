...
def createGaussianPyramid(im, sigma0=1, k=np.sqrt(2),
                          levels=[-1, 0, 1, 2, 3, 4]):
    if(len(im.shape) == 3):
    ...
    for l in levels:
        sigma = sigma0 * k ** l
        GaussianPyramid.append(cv2.GaussianBlur(im, (0, 0), sigma))
    GaussianPyramid = np.stack(GaussianPyramid, axis=2)
...
def displayPyramid(im_pyramid):
    ...
    im_pyramid = cv2.normalize(
        im_pyramid, None, alpha=0, beta=1,
        norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    # cv2.imshow('test.png', im_pyramid)
    # cv2.waitKey(0)  # press any key to exit
    # cv2.destroyAllWindows()
    plt.imshow(im_pyramid)
    plt.show()
...
def createDoGPyramid(gaussian_pyramid, levels=[-1, 0, 1, 2, 3, 4]):
    ...
    for l in range(len(levels) - 1):
        DoG_pyramid.append(
            gaussian_pyramid[:, :, l + 1] - gaussian_pyramid[:, :, l])
...
def computePrincipalCurvature(DoG_pyramid):
    ...
    INPUTS
        DoG Pyramid - size (imH, imW, len(levels) - 1)
        matrix of the DoG pyramid

    OUTPUTS
        principal_curvature - size (imH, imW, len(levels) - 1)
        matrix where each
    ...
    for l in range(DoG_pyramid.shape[-1]):
        ...
        # in case det == 0
        det[det == 0.] = 10**(-20)
        principal_curvature.append((Dxx + Dyy)**2 / det)
...
def getLocalExtrema(DoG_pyramid, DoG_levels, principal_curvature,
    ...
    for i in range(0, l):
        # find neighbors
        neighbors = [DoG_pyramid[:imH - 2, :imW - 2, i],
                     DoG_pyramid[:imH - 2, 1:imW - 1, i],
                     DoG_pyramid[:imH - 2, 2:, i],
                     DoG_pyramid[1:imH - 1, :imW - 2, i],
                     DoG_pyramid[1:imH - 1, 2:, i],
                     DoG_pyramid[2:, :imW - 2, i],
                     DoG_pyramid[2:, 1:imW - 1, i],
                     DoG_pyramid[2:, 2:, i]]
        if i > 0:
        ...
        P = principal_curvature[1:imH - 1, 1:imW - 1, i]
        # find local extrema
        is_extrema = (layer_data > np.max(neighbors, axis=0)) | (
            layer_data < np.min(neighbors, axis=0))
        # apply threshold to exclude unqualified points
        is_extrema &= (np.absolute(layer_data) > th_contrast)
        is_extrema &= (np.absolute(P < th_r))
        spatial_loc = np.where(is_extrema)
...
def DoGdetector(im, sigma0=1, k=np.sqrt(2), levels=[-1, 0, 1, 2, 3, 4],
    ...
    gauss_pyramid = createGaussianPyramid(
...
if __name__ == '__main__':
    ...
    cv2.imwrite('../keypoints.jpg', tmp_im)
